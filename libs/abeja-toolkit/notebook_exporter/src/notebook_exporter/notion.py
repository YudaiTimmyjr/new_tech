...
from notion_extension.blocks import Block, Blocks
from notion_extension.client import Client
from notion_extension.db_properties import Properties
from notion_extension.objects import Block as _Block
from notion_extension.utils import make_batch
...
class NotionExporter:
    ...
    def _to_notion_blocks(self, cells: list[Cell], drive_dir_id: str | None) -> list[_Block]:
        blocks = []
        flag_warning = False
        for cell in cells:
            if cell.type == "output":
                ...
                if mime_type in {"image/jpeg", "image/png"}:
                    ...
                    blocks.append(
                        Block.code(content=cell.text or "", language="plain text", caption="output")
                    )
    ...
    def export(
        ...
        for i, batch in enumerate(make_batch(blocks, batch_size=1)):
            if i == 0:
                page = cast(dict[str, Any], self.notion_client.create_page(
