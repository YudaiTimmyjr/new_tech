...
You can install this tool by `pip` or `uv` after checkout of the code.

```shell
uv add --editable /path/to/spreadsheet
...
### Get a partial worksheet as a pandas dataframe

```python
spreadsheet.as_dataframe(
    worksheet_name="<worksheet name to fetch>",
    range_name="<range name (e.g., 'A1:D8')>,
    header_row=<number of header row relative to the range (e.g., 0)>
)
```
...
```python
spreadsheet.get_worksheet_names()
```
